...
## TODO:
url_custom_sheet <- "https://docs.google.com/spreadsheets/d/1FdXUX5LqM0hB8iL78IAXFFmJEauuwm0Yv3KUDE4ACDM/edit?resourcekey=&gid=46999762#gid=46999762"

df_hours <- read_sheet(url_custom_sheet)
df_hours %>% glimpse()
```
```{r}
df_long <- df_hours %>%
  pivot_longer(
    cols = contains("what day"),
    names_pattern = "\\[(\\w+)\\]",
    names_to = "day",
    values_to = "hours"
  ) %>%
  ...
  unnest(hours) %>%
  na.omit() %>%
  mutate(hours = str_trim(hours),  # Remove leading/trailing whitespace
         hours = toupper(hours)) %>% 
  mutate(day = fct_relevel(day, "monday", "tuesdya", "wednseday", "thrusday", "friday", "saturday", "sunday" ))


df_counts <- df_long %>%
  count(day, hours)

df_counts <- df_counts[order(df_counts$day),]
df_counts
```
...
ggplot(df_counts, aes(x = day, y = hours, fill = n)) +
