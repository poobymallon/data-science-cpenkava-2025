...
## TODO: Pivot the data, rename the columns
  # Pivot longer to create tidy format
  pivot_longer(
...
- Document your observations here.
  - the number of people in a family generally tends to correlate with a
    higher median income
    - this kind of breaks down in certain categories especially for 6
      person families
  - a lot of the 4 and 6 person families have wide confidence intervals
  - a lot of 2 person families have really small confidence intervals
  - we cannot conclude for certain any inequalities between 2-6 person
    families because almost all of the confidence intervals overlap
  - we cannot tell if any counties have higher costs of living
  - (Your response here)
  - …
- Can you confidently distinguish between household incomes in Suffolk
  county? Why or why not?
  - Just barely - the confidence intervals are all quite similar in
    width and are centered very close to each to the point they just
    kind of blur together
- Which counties have the widest confidence intervals?
  - Nantucket is superrrrr wide and then Dukes is also pretty wide
...
df_data %>%
  ...
  labs(
    ...
    y = "Standard Error of Median Income (log scale)",
    title = "Uncertainty in Income Estimates Decreases with Population",
    subtitle = "Smaller counties have higher relative uncertainty in income estimates"
  ) +
  theme_minimal()
```
...
- What *overall* trend do you see between `SE` and population? Why might
  this trend exist?
  - Generally, the larger the population, the lower the standard error -
    I would assume this is because standard error divides by the square
    root of n as part of the equation
- What does this *overall* trend tell you about the relative ease of
  studying small vs large counties?
  - This would mean that smaller counties are harder to study because
    they are more varied, so you have to get larger samples to have the
...
df_cv_by_state <- df_data %>%
  ...
  group_by(state) %>%
  summarize(
    median_SE = median(income_SE, na.rm = TRUE)
  ) 

# Plot top 10 states with highest uncertainty
df_cv_by_state %>%
  slice_max(median_SE, n = 10) %>%
  mutate(state = fct_reorder(state, median_SE)) %>%
  ggplot(aes(state, median_SE)) +
  geom_col(alpha = 0.8) +
  coord_flip() +
  theme_minimal()
```
...
- The states with the highest SE are North Dakota, Wyoming, Montana,
  Alaska, and South Dakota
  - This is highly in line with our graph from earlier showing that SE
    is inversely correlated with population!
  - Nebraska has a decently high SE - that’s where I’m from!
