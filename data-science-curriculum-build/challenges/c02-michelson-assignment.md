...
My distinctness 3 is 1.7 more, my distinctness 2 is 1.5 less, and my
distinctness 1 is 2 less. We did the same thing to our data - sort it by
distinctness and then take the average, which means that Michelson must
have done some additional step to the data. Maybe he adjusted for
environmental factors (perhaps why temperature is included in the data)
or error from the “crude testing setup” that the reading described.
...
Michelson’s estimate of possible error, found in LIGHTSPEED_PM, is
actually about 3 times less than the actually error between his
calculation. His error estimate is less than the true error.

Make a quantitative comparison between Michelson’s uncertainty and his
error. -

Because his assessment was about 3 times smaller than the actual error,
it is fair to say that Michelson severely underestimated how wrong he
could be.
...
- similar shape of the overall line - not super volatile in behavior,
  generally follow each others ups and downs

- lines mostly dont deviate from the bounds - the average behavior of
  both, dictated by the line don’t typically deviate from the error
  bounds, and when they do, it’s not by much

Differences:

- number of points outside the bounds - the simulated has more points
  outside the dashed line, and not just by a few

- clustering - each day in the real data seems to have a cluster of
  points together for that day, while the simulated seems a bit more
  spread out with less clustering of points. this is specifically
  notable in the July 16-23 region
...
df_q2 %>% 
  ...
  geom_point(mapping = aes(color = Distinctness))
```
...
- Plot 1:
  - There is not a clear trend here - there are some outliers in the
    corners that might make a regression model read a positive
    correlation, but I’m not sure that “positive correlation” would be a
    fully accurate statement to make.
  - It is interesting to see that the Distinctness 1 data lives in sort
    of two groups, Distinctness 2 is all over the board, and
    Distinctness 3 tends to be in the higher temperature range.
  - There is a visible gap in the high 70s temperature range. I wonder
    what kind of data would have shown up there?
- Plot 2:
  - I looked ahead at the boxplot exercise for this one!
  - The medians do technically go up as the distinctness goes up, but
    the boxes have a lot of overlap with each other value wise.
  - The median for Distinctness 2 data is closest to the actual value…
    Not either extreme, which I find odd - I would expect that either
    higher temperature or lower temperature air would be most similar to
    a vacuum, not something in the middle.
- Plot 3:
  - Another visualization of data similar to plot 1, but a little
    different info
  - Interesting to see that all the Distinctness 1 data are from the
    same few days - maybe they were some off days. Distinctness 2 and 3
    data are spread across the days such that they don’t raise any
    alarms visually.
  - Again, no clear trend here, but that makes sense - the speed of
    light shouldn’t change over time, or at least I would hope so.
- Bonus Plot 4:
  - If you’re curious about when all of the data was collected - this
    graph shows how many data points were captured each day - this guy
    made sure to take breaks and not burn himself out :)
