...
- Is this sample representative of *all* houses in Helvig? Why or why
  not?
  - No! This was only a sample of the first 25, so it can’t be
    representative of all houses. What if Sorensen family chose to live
    near each other, which is why they show up most in the first 25, but
    behind the scenes they don’t show up in any of the other 500 some
    houses?
...
- Which sample—sequential or random—is more *representative* of all
  homes Helvig? Why?
  - random
  - as I stated earlier, just picking the first n fails to capture any
    possible geographic differences within the people - picking a random
    sample means that each person at least has a chance to be picked
...
- Is there any reason that people with the same last name might tend to
  *live near each other*?
  - Oh my gosh, I’m so glad you’ve asked. As stated before, humans tend
    to attempt to live next to the people they like and the people they
    visit most often for convenience reasons. This would lead to the
    case that many families (people typically with the same last name as
    each other) may try their best to live as close as possible to each
    other.
...
```
    ...
    ## 1 prevalence 0.0172    0.0715  0.140   0.05 percentile
...
- Note that we used the *random* sample with the bootstrap procedure in
  ...
  - no
  - I still think that it would not be representative because it doesn’t
    take into account the geographic variable. The geographic variable
    could be a large factor of covariance, so it could potentially skew
    the bootstrapping. There is bias in the initial sample, so this will
    show up in the bootstrap samples we draw from it.
...
- Our study is purely observational because we will just collect their
  data at point of observation. They will not be doing anything for us.

#### Question / Hypothesis

- How are wage and gender/age related in the population? Specifically,
  do any patterns show up in the farming community?
...
alternatively, here is what my team came up with after I worked with
them on this, seeing as that is how the project is going to go, so the
one I made above doesn’t mean that much:
