...
assertthat::assert_that(
    tibble(x = .25, y = .25) %>% # Pick a point *inside* the circle
...
- You chose a correct value of `stat(x, y)` when `x, y` is *inside* the
  ...
  - I didn’t want to choose 0,0 because I figured that wouldn’t be a
    very meaningful point, so I chose a non-zero point most definitely
    in the circle, .25, .25
...
- What is a range of plausible values, based on the sampling
  ...
  - The range of all values given by this method is from about 2.95 to
    3.38
  - If we visual think about our 2 z scores out, then I would give a 95%
    confidence interval of like 3.05-3.3 ish
...
df_q5 <- 
  ...
  summarize(
    ...
    pi_lo = quantile(pi_est, .025),
    pi_up = quantile(pi_est, .975)
  )
...
- How closely do your bootstrap CI and CLT CI agree?
  - (Your answer here) They are quite close! 3.096 vs. 3.0966 and 3.292
    vs. 3.295 - very close!
- Comment on the width of your CI(s). Would your estimate of $\pi$ be
  ...
  orbit)?
  - (Good enough as a rough estimate?) The width is pretty wide for
    being exact, but for the example of painting a circle, you could
    just go on the safe side
  - (Good enough as a precise estimate?) but… for precise things, you
    would want a much more precise idea of the actual value (duh), so
    this range would be far too wide for something like a rocket in
    orbit
