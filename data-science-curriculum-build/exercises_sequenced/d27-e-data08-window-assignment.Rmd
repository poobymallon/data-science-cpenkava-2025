...
economics %>%
  mutate(
    trend = ifelse(unemploy < lead(unemploy, order_by = date), "increasing",  "decreasing")
    ) %>%
  ggplot(aes(date, unemploy)) +
  geom_segment(aes(
    xend = lead(date, order_by = date),
    yend = lead(unemploy, order_by = date),
    color = trend  # Color segments based on the trend
  )) +
  scale_color_manual(values = c("increasing" = "green", "decreasing" = "red"))  # Green for increasing, red for decreasing
```
...
q2 <- mpg
q2 <- q2 %>%
  mutate(rank = dense_rank(-hwy)) %>%  # Rank by 'hwy', descending order
  filter(rank <= 3)
q2
```
