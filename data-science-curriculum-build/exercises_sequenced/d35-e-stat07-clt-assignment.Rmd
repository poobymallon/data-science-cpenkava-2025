...
df_clt <-
  # [OUTER LOOP] Generate a sample n_repl times
  map_dfr(
    1:n_repl,
    function(id) {
      # [INNER LOOP] Generate samples of sizes: 1, 2, 9, 81, 729
      map_dfr(
        c(1, 2, 9, 81, 729),
        function(n) {
          tibble(
  ...
  # IMPORTANT END PART: COMPUTE AVERAGE
  group_by(n, id) %>%
...
lo_q1 <- 1-qnorm( 1 - (1 - 0.99) / 2 )*2/ sqrt(100)
hi_q1 <- 1+qnorm( 1 - (1 - 0.99) / 2 )*2/ sqrt(100)
