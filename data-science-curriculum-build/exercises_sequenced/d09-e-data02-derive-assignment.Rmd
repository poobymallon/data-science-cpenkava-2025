...
-   Comment on the relative values of `cov_depth` and `cov_diff`.
    -   very, very different - cov diff is huge and honestly should not be that big, and cov_depth is very small, which is pretty reasonable since the mean value is in the 100s
-   Why is `cov_diff` so large?
    -   cov_diff is so large bc it seems the difference wildly varies (around -10 to 10) but the mean is around zero, so it's probably like a standard deviation of like 3 or 4 divided by a mean of like .02 or something
