...
df_q3 <- 
  flights %>% 
  filter(arr_time != sched_arr_time & near(arr_time, sched_arr_time, tol = 10)
  )

df_q3 %>% select(sched_arr_time, arr_time, everything())
```
...
```{r q5-task}
BOS <- "BOS"
flights %>% filter(dest == BOS) %>% select(dest, everything())
```
...
df_q6a %>% select(dest, everything())
df_q6b %>% select(origin, everything())
```
...
**Observations** - Was this dataset assembled to study flights *out of* NYC, or *into* NYC? How do you know? - out of: there was only a single data point where an NYC flight was the destination
...
df_q7 %>% select(air_time, dest, everything())
```
