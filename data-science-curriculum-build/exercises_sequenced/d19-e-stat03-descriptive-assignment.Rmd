...
flights %>%
  summarize(dep_delay = mean(na.omit(dep_delay)))
```

**Observations**:

-   What is the mean departure delay time, among available values? 12.64 minutes about.
...
-   What do negative values of `dep_delay` mean?
    -   planes leave early, which can only happen if all the passengers board very punctually.
