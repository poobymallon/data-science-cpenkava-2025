...
- the orders are wrong, so it seems that bind just assumes the orders are the same for both
...
```{r q2-task}
df_q2 <-
  inner_join(beatles1, beatles3, by = "name") %>% 
...
airport_codes <- 
 airports %>% 
  filter(
    # name %in% c("San Jose Airport", "San Francisco", "Metropolitan Oakland")
